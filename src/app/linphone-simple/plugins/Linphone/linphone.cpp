...
void Linphone::enableSpeaker() {
    QProcess pacmdProcess;
    QStringList args;
    args << "set-sink-port" << "0" << "output-speaker";

    pacmdProcess.start("pacmd", args);
    pacmdProcess.waitForFinished();  // This shouldn't take too long
}

void Linphone::disableSpeaker() {
    QProcess pacmdProcess;
    QStringList args;
    args << "set-sink-port" << "0" << "output-earpiece";
...
void Linphone::displayOn() {
    QProcess powerdCliProcess;
    QStringList args;
    args << "display" << "on";

    powerdCliProcess.start("powerd-cli", args);
    //Let's wait 3/4 of a second to be «finished» otherwise screen doesn't turn on (as QProcess is killed before)
    powerdCliProcess.waitForFinished(750); 
}
