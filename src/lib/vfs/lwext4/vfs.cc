...
class Vfs::Lwext4_vfs_file_system : public Vfs::File_system
{
		...
		struct Lwext4_vfs_dir_handle : Lwext4_vfs_handle
		{
			                          ...
			                          file_size &out_count) override
			{
				...
				if (dir_index < cur_index) {
					dir.next_off = dir_index > 0 ? dir_index : 0;
					cur_index = 0;
				}
	...
	public:
			...
			_io_truncation { io_truncation }
		{
			...
			ext4_blockdev *bd = &block_device.ext4_blockdev;
		...
		Write_result write(Vfs_handle *vfs_handle,
		                   ...
		                   file_size &out_count) override
		{
			Lwext4_vfs_handle *handle =
				static_cast<Lwext4_vfs_handle *>(vfs_handle);
		...
		Read_result complete_read(Vfs_handle *vfs_handle, char *buf,
		                          ...
		                          file_size &out_count) override
		{
			Lwext4_vfs_file_handle *handle = static_cast<Lwext4_vfs_file_handle *>(vfs_handle);
		...
		Ftruncate_result ftruncate(Vfs_handle *vfs_handle, file_size len) override
		{
			...
			if (fsize < len) {
				if (!_io_truncation)
					...
					file_size const amount = len - fsize;

					for (file_size increase = amount; increase && increase <= amount; ) {
						file_size out_count;

						file_size const current_size =
