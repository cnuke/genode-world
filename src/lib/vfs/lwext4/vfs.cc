...
class Vfs::Lwext4_vfs_file_system : public Vfs::File_system
{
	...
	public:
		...
		Sync_result complete_sync(Vfs_handle *vfs_handle) override
		{
			...
			/*
			 * For as of now unknown reasons the journal will slowly
			 * leak memory. Stopping and restarting the journal,
			 * however, will free up the used memory so we do that
			 * as an interim fix.
			 */
			{
				...
				if (++count >= 31) {
					...
					err = ext4_journal_start(_fs_mp);
					if (err)
						error("could not re-start journal, err: ", err);
				}
			}
