...
static bool perfom_blocking_io(Genode::Entrypoint                   &ep,
...
struct Block_io: Util::Io
{
	...
	Block_io(Genode::Entrypoint &ep, Vfs::Vfs_handle &vfs_handle)
	:
		_ep         { ep },
	...
	Write_result write(Buffer const &src, At const at) override
	{
		...
		bool const success = perfom_blocking_io(_ep, _vfs_job, _vfs_handle,
                       ...
                       bool                  write)
{
	...
	if (!bd._cache) {
		...
		Util::Io::Buffer buffer { .base = reinterpret_cast<uint8_t*>(buf),
		                          .length = length };
		return perfom_blocking_io(ep, job, handle, op, offset, buffer) ? EOK : EIO;
	}
...
void Lwext4::block_sync(Block_device &block_device)
{
	...
	Genode::Entrypoint &ep     =  block_device._vfs_env->env().ep();
